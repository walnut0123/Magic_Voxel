...
public static class ARAVRInput  //입력을 받도록
{
#if PC
    public enum ButtonTarget //숫자로 치면 012 로 이루어진 숫자(리스트와 유사함)
    {
        Fire1,
        Fire2,
        Fire3, //마우스 휠
...
#elif Oculus //메타 퀘스트 SDK에서 처리하고 있는 인풋
...
#endif
#if Oculus
    static Transform GetTransform() //정보를 받아온다, get > 값을 받는다( 접근자 ) ,set > 값을 넣는다 
    {
...
# endif
    ...
    public static Vector3 RHandPosition
    {
        get
        {
            ...
            return pos;
#elif Oculus
            Vector3 pos = OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch); //오큘러스는 바로 값을 받아오기만 하면 됨
    ...
    public static Vector3 RHandDirection
    {
        ...
        {
#if PC
            Vector3 direction = RHandPosition - Camera.main.transform.position; //기하와 벡터 파트.,원점~카메라, 원점~핸드 를 했을 때 핸드 - 카메라 = 방향성을 띔.
